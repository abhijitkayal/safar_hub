...
type AdventuresExplorerProps = {
  category?: string; // 👈 comes from /adventures/[category]
};
...
export const AdventureCard = ({
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-3xl border bg-white shadow hover:-translate-y-1 transition"
    >
      <div className="relative h-56 w-full">
        {adventure.images?.[0] && (
          <Image
      ...
      <div className="flex flex-col gap-3 p-5">
        ...
        <div className="mt-auto flex items-center justify-between">
          <div>
            ...
            {startingPrice && (
              <p className="text-xs text-gray-500">From ₹{startingPrice}</p>
            )}
          </div>
          ...
          <button
            ...
            className={`rounded-full p-2 ${
              isInCart ? "bg-green-600 text-white" : "bg-green-100 text-green-700"
            }`}
          >
            <FaShoppingCart />
          </button>
...
export default function AdventuresExplorer({ category }: AdventuresExplorerProps) {
  const router = useRouter();

  const normalizedCategory: AdventureCategoryValue =
    ADVENTURE_CATEGORIES.some((c) => c.value === category)
      ? (category as AdventureCategoryValue)
      : "all";

  const [adventures, setAdventures] = useState<Adventure[]>([]);
  const [activeCategory, setActiveCategory] =
    useState<AdventureCategoryValue>(normalizedCategory);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch("/api/vendor/adventures?all=true", {
          cache: "no-store",
        });
        const data = await res.json();
        setAdventures(data.adventures || []);
  ...
  /* ===== CATEGORY CHANGE ===== */
  const handleCategoryChange = useCallback(
    (value: AdventureCategoryValue) => {
      setActiveCategory(value);
      if (value === "all") {
        router.replace("/adventures", { scroll: false });
      } else {
        router.replace(`/adventures/${value}`, { scroll: false });
      }
    },
    [router]
  );
  ...
  const filteredAdventures = useMemo(() => {
    return adventures.filter(
      (adv) => activeCategory === "all" || adv.category === activeCategory
    );
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        ...
        {loading ? (
          ...
          <div className="mt-10 text-center">
            <p className="text-gray-600">No adventures found for this category.</p>
            <p className="text-sm text-gray-500 mt-2">
              Total adventures loaded: {adventures.length} | Active category: {activeCategory}
            </p>
          </div>
        ) : (
          <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {filteredAdventures.map((adv) => {
              const isInCart = cartItems.some(
                (i) => i.itemId === adv._id && i.itemType === "Adventure"
              );

              const handleCart = async (e: React.MouseEvent) => {
                e.preventDefault();
                if (isInCart) {
                  const item = cartItems.find((i) => i.itemId === adv._id);
